...
/**
 * Class CommitsPerWeekday
 *
 * Git commits over time (by weekday)
 *
 * @package Extension\Analysis\Analysis\Git
 */
class CommitsPerWeekday extends \Extension\Analysis\Analysis\Base {
    ...
    public function generate() {
        ...
        $this->setJavaScriptFiles(array('Git/CommitsPerWeekday.js'));
    ...
    private function execDataQuery($configuration) {
        ...
        $uidIndexField = 'weekDay';
        $select = '
            COUNT(scmlog.id) AS cnt,
            DATE_FORMAT(scmlog.date, \'%w\') AS weekDay';
    ...
    private function prepareData(array $dataSets) {
        // Sunday is 0 in MySQL
        // Move Sunday to the end of the week :)
        $dataSets[7] = $dataSets[0];
        unset($dataSets[0]);
